...
namespace Alfred.Core.WebApi.Configuration;
...
internal sealed class OptionalJsonConverter<T> : JsonConverter<Optional<T>>
{
    ...
    {
        // This method is invoked only when the JSON property key is present.
        // If the key is not in the JSON payload, the struct keeps its default (HasValue = false).
