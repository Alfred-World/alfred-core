...
[Route("core/v{version:apiVersion}/account-sales/products")]
[Authorize]
public sealed class AccountSalesProductController : BaseApiController
{
    ...
    {
        var result = await _service.SearchProductsAsync(query.ToSearchRequest(), cancellationToken);
    ...
    {
        var result = await _service.GetProductByIdAsync((ProductId)id, cancellationToken);
        if (result is null)
        {
    ...
    public async Task<IActionResult> UpdateProduct(Guid id, [FromBody] UpdateProductRequest request,
        CancellationToken cancellationToken)
    {
        var result = await _service.UpdateProductAsync((ProductId)id, request.ToDto(), cancellationToken);
        return OkResponse(result);
    }
}
