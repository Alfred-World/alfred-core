...
public sealed class AccountSalesOrderController : BaseApiController
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await _service.SellAccountAsync(request.ToDto(), sellerUserId, cancellationToken);
    ...
    public async Task<IActionResult> ReplaceAccount(Guid orderId, [FromBody] ReplaceAccountOrderRequest request,
        CancellationToken cancellationToken)
    {
        var result = await _service.ReplaceAccountForWarrantyAsync(orderId, request.ToDto(), cancellationToken);
